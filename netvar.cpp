...
void netvar::ListClassNames() {
  std::cout << "NetVar Classes: " << std::endl;
  for(size_t i = 0; i < g_classes.size(); i++) {
      std::cout << "Class Name: " << g_classes[i].name << std::endl;
  }
}

void netvar::ListClassProps(std::string classname) {
  for(size_t i = 0; i < g_classes.size(); i++) {
      if(g_classes[i].name.compare(classname) != 0)
          continue;
      std::cout << "NetVar Class: " << g_classes[i].name << std::endl;
      for(size_t p = 0; p < g_classes[i].props.size(); p++) {
          std::cout << "Prop Name: " << g_classes[i].props[p].name << std::endl;
      }
  }
}
